...
## Zuverlässiges Timing (Reveal + optionaler cron-job.org-Booster)

Die vier Ausgaben (06/11/16/20 Uhr Wien) + die Wochen-Quittung laufen in GitHub
Actions. Damit sie **punktgenau** in der Webapp stehen, gilt seit dem Umbau „Früher
Trigger + exakte Enthüllung":

- Die GitHub-Crons feuern **früh & häufig** (15-Min-Takt ab ~40 min vor dem Slot),
  sodass das Lagebild i. d. R. **vor** der Kuratierungszeit erzeugt und deployt ist;
  die genaue Ausgabe wählt `python -m copilot.main auto` zeitzonensicher aus der
  Wiener Uhrzeit (nicht der UTC-Stunde).
- Die Webapp **enthüllt** die neueste Ausgabe taktgenau zur Slot-Sekunde
  (`data-publish` + Reveal-JS) — nie zu früh; ist GitHub doch zu spät, erscheint sie
  sofort nach dem Deploy. Das funktioniert **repo-only**, ohne externen Dienst.

### Optionaler Booster (garantiert pünktliche Erzeugung)

GitHubs interner Schedule-Cron feuert grundsätzlich unpünktlich. Wer die Erzeugung
fest an den Slot binden will, schaltet zusätzlich einen **externen Trigger** scharf,
der die Workflows per `workflow_dispatch`-API anstößt (startet <1 min):
...
3. Script ausführen — es legt **9 Jobs** in der Zeitzone *Europe/Vienna* an (5 Presse
   via `lagebild.yml` + 4 NYT via `nyt.yml`, ganzjährig pünktlich, kein DST-Drift) und
   benennt je Job die Ausgabe explizit über den `edition`-Input. Jeder Job feuert
   ~10 min **vor** dem Slot (NYT 2 min versetzt), passend zur Enthüllung. Erneutes
   Ausführen ist idempotent.
4. Status prüfen: `CRONJOB_KEY=<…> bash scripts/setup_cron.sh verify` listet die Jobs
   samt letztem Lauf. End-to-End: auf cron-job.org bei einem Job *„Run now"* → in
   GitHub Actions erscheint <1 min ein `workflow_dispatch`-Lauf, Telegram-/Web-Push
   kommt sofort.

Secrets (PAT, API-Key) **niemals** ins Repo committen — nur zur Laufzeit übergeben;
der PAT lebt allein in den cron-job.org-Job-Headern. Die GitHub-Schedule-Crons in den
Workflows bleiben als Best-Effort-Fallback erhalten.
